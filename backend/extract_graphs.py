...
def extract_graphs():
    ...
    for row in rows:
        ...
        try:
            ...
            # Extract mode from base_title (e.g. 'Coco_2017_unigram' -> 'unigram')
            mode = base_title.split(' ')[-1]
            
            # Filter results by mode
            filtered_results = [res for res in k_results if res.get('mode') == mode]
            
            # Fallback if no modes match (just in case old data doesn't have mode)
            if not filtered_results:
                filtered_results = k_results

            # Extract data
            k_values = [res['k'] for res in filtered_results]
            coherence_scores = [res['score'] for res in filtered_results]
            perplexity_scores = [res['perplexity'] for res in filtered_results]
